# SPEC VERSION: ai-agent-v2
# STATUS: Production-Ready
# COMPATIBLE WITH: SPEC_tgbot_codex_v18.md
# TARGET PLATFORM: Google Cloud Run
# LAST UPDATED: 2026-01-28
# CHANGES FROM v1: Added voice message processing (POST /api/voice)
...
Compatible with Telegram Bot defined in SPEC_tgbot_codex_v18.md.
Supports both text and voice message processing.
...
- Accepts user messages via HTTP (text and voice)
- Processes natural language input
- Transcribes voice messages and generates responses
...
- MODEL_NAME defaults to "gemini-2.0-flash" if missing
...
- MUST NOT log API keys, full message content, or audio content (OK: session_id, message length, audio size)
...
- POST /api/chat -> Text message processing (see CHAT API CONTRACT)
- POST /api/voice -> Voice message processing (see VOICE API CONTRACT)

---

## CHAT API CONTRACT (MANDATORY)
...
Rules:
- MUST return HTTP 200 on success
- MUST return HTTP 400 on missing/invalid required fields
- MUST return HTTP 500 on processing error with {"error": "Agent unavailable, please try again later"}
- MUST complete within 30 seconds (matching bot timeout)
- MUST be safe to retry (stateless)
...
- If API key not configured: return 200 with {"response": "AI model not configured...", "transcription": ""}
...
- async generate_response_from_audio(self, audio_base64: str, mime_type: str, session_id: str) -> dict
  - If api_key is None -> return {"response": "AI model not configured. Please contact administrator.", "transcription": ""}
  - Build Gemini multimodal request with inline_data containing audio
  - Include prompt instructing model to transcribe and respond with [TRANSCRIPTION] and [RESPONSE] markers
  - Parse response using _parse_voice_response()
  - On success: return dict with "response" and "transcription" keys
  - On API error: raise RuntimeError with masked error
  - MUST NOT log audio content
  - MUST include timeout protection

- @staticmethod _parse_voice_response(raw_text: str) -> dict
  - If both [TRANSCRIPTION] and [RESPONSE] markers present:
    - Extract transcription between [TRANSCRIPTION] and [RESPONSE]
    - Extract response after [RESPONSE]
  - Else: return {"response": raw_text.strip(), "transcription": ""}
  - MUST NOT raise on unexpected format

### Gemini Multimodal Request Body (MANDATORY):
  ...
  "contents": [
    {
      "parts": [
        ...
        {
          "text": "You received a voice message. First, transcribe the audio exactly as spoken. Then, respond to the content naturally.\n\nFormat your reply EXACTLY as:\n[TRANSCRIPTION]\n<exact transcription>\n[RESPONSE]\n<your response>"
        }
      ]
    }
  ]
...
- async process_voice(self, session_id: str, audio_base64: str, mime_type: str) -> dict
  - Validate non-empty audio_base64
  - If empty: return {"response": "Empty audio received. Please send a voice message.", "transcription": ""}
  - Call llm_client.generate_response_from_audio(audio_base64, mime_type, session_id)
  - Return dict with "response" and "transcription" keys
  - On any exception: raise RuntimeError with user-friendly message
...
- Routes for /health, /healthz, /api/chat, and /api/voice
...
- Deploy:
  ...
  - MUST NOT set PORT in env vars (Cloud Run injects PORT automatically)
  - MUST set secrets:
    --set-secrets "MODEL_API_KEY=<secret_name>:latest"
...
- Mention support for text and voice messages

2) Architecture:
- Single process: uvicorn + FastAPI + LLM client
- Lifespan controls LLM client lifecycle
- Stateless request processing
- Gemini multimodal API for voice transcription
...
- GET /healthz - Health check (Cloud Run alias)
- POST /api/chat - Text message processing
- POST /api/voice - Voice message processing (transcription + response)

5) Local development:
- cp .env.example .env
- set MODEL_API_KEY (or explain Secret Manager usage)
- ./deploy-agent-local.sh
- Verify: curl /health and POST to /api/chat and /api/voice
...
- Logs do not contain secrets/message bodies/audio content
...
- test_voice_endpoint_valid_contract: POST /api/voice returns {"response": ..., "transcription": ...}
- test_voice_endpoint_missing_api_key: API key None -> "not configured" response
- test_voice_endpoint_missing_audio: Missing audio_base64 -> 400 error
- test_voice_endpoint_missing_session_id: Missing session_id -> 400 error
- test_voice_endpoint_invalid_json: Malformed request -> 400 error
- test_voice_endpoint_timeout: Mock timeout -> 500 error
...
- test_llm_client_voice_no_api_key: Voice with no API key -> returns not configured
- test_llm_client_voice_success: Mock voice API call -> returns response + transcription
- test_parse_voice_response_with_markers: Parse [TRANSCRIPTION] and [RESPONSE] markers
- test_parse_voice_response_fallback: Parse without markers -> full text as response
...
6. agent/config.py (with key extraction, sanitization, mask_token)
7. logging setup (JSON + trace middleware in app.py)
8. agent/llm_client.py (with text + voice methods, timeout, masking, error handling)
9. agent/processor.py (with text + voice methods)
10. app.py (lifespan + routes for /health, /healthz, /api/chat, /api/voice + trace middleware)
...
15. README.md
16. tests (health, chat, voice, llm_client)
17. self-checks
18. STOP
...
- deploy-agent.sh does NOT set PORT in --set-env-vars
...
- Error messages mask API keys via mask_token()
...
- Both text and voice messages processed correctly
